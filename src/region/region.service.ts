...
export class RegionService {
  ...
  async createRegion(dto: CreateRegionDto): Promise<{ id: number }> {
    const candidate = await this.getRegionByValue(dto.value);
    if (candidate) {
      throw new RegionExistException();
    }
    const region = await this.regionRepository.save({ title: dto.value });
    return { id: region.id };
  }
  ...
  async createCity(dto: CreateCityDto): Promise<{ id: number }> {
    const candidate = await this.cityRepository.findOne({ where: { title: dto.value } });
    if (candidate) {
    ...
    const city = await this.cityRepository.save({ title: dto.value, region });
    return { id: city.id };
  }
